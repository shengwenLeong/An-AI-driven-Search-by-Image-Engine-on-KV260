...
#pragma omp parallel for
    ...
    public:
        ...
        virtual void save (char const *path, int format) const {
            ...
            cout<<"sizeof(SIGNATURE_VERSION)---"<<sizeof(SIGNATURE_VERSION)<<endl;
            cout<<"sizeof(sig_cap)--"<<sizeof(sig_cap)<<"--"<<reinterpret_cast<char const *>(&sig_cap)<<endl;
            cout<<"sizeof(N)--"<<sizeof(N)<<"--"<<reinterpret_cast<char const *>(&N)<<endl;
            ...
            for (unsigned i = 0; i < graph.size(); ++i) {
                ...
                if (format & FORMAT_NO_DIST) {
                    ...
                    //cout<<"nids sizeof --"<<sizeof(nids[0])<<endl;
                }
                else {
                    os.write(reinterpret_cast<char const *>(&knn[0]), K * sizeof(knn[0]));
                    //cout<<"knn[0] size --"<<sizeof(knn[0])<<endl;
                    //cout<<"sizeof(struct Neighbor)--"<<sizeof(bool)<<endl;
                    for (unsigned k=0; k < K; ++k){
                        //cout<<" "<<knn[k].id;//<<"-"<<knn[k].dist;
                    }
                }
                //cout<<endl;
            }
...
#pragma omp parallel for
        ...
        */
        ...
        virtual unsigned search (SearchOracle const &oracle, SearchParams const &params, unsigned *ids, float *dists, SearchInfo *pinfo) const {
            ...
            vector<vector<NeighborX>> cache;
            vector<unsigned> cache_index;
            ...
            for (unsigned trial = 0; trial < params.T; ++trial) {
                ...
                for(unsigned k=0;k<L;++k) {
                    ...
                    //cache.push_back(graph[e.id]);
                    cache_index.push_back(e.id);
                    //cout<<neighbors.size()<<endl;
                    for(unsigned m = 0; m<neighbors.size();++m){
                        cache_index.push_back(neighbors[m].id);
                        auto const &neigh = graph[neighbors[m].id];
                        for(unsigned n=0;n<neigh.size();++n){
                            cache_index.push_back(neigh[n].id);
                            auto const &nei = graph[neigh[n].id];
                            for(unsigned l=0;l<nei.size();++l){
                                cache_index.push_back(nei[l].id);
                            }
                        }
                    }
                }
                //cout<<"Cache size"<<cache_index.size()<<endl;
                /*for(auto iter=knn.begin();iter != knn.end();++iter)
                {
                    cout<<"knn----"<<(*iter).id<<"--"<<(*iter).dist<<"--e.M--"<<(*iter).M<<endl;
                }*/
                ...
                for(unsigned k=0;k<L;++k){
                    auto &e = knn[k];
                    //cout<<e.id<<"+dist="<<e.dist<<endl;
                    
                }
                //knn.resize(L);
                /*vector<NeighborX> knn_temp;
                knn_temp.resize(knn.size());
                copy(knn.begin(),knn.end(),knn_temp.begin());
                //knn.clear();
                copy(knn_temp.begin(),knn_temp.end(),knn.begin());
                cout<<"--------------------------------------------------------"<<endl;*/
                /*for(auto iter=knn.begin();iter != knn.end();++iter)
                {
                    cout<<"knn----"<<(*iter).id<<"--"<<(*iter).dist<<"--e.M--"<<(*iter).M<<endl;
                ...
                while (k < L) {
                    ...
                    e.m = endM;
                    temp = 0;
                    vector<unsigned>::iterator s=find(cache_index.begin(),cache_index.end(),e.id);
                    //if(s==cache_index.end()) cout<<"cache miss"<<endl;
                    //cout<<"---e.id="<<e.id<<"e.m="<<e.m<<"-e.M="<<e.M<<"--"<<beginM<<"----"<<endM<<"---------------"<<endl;
                    ...
                    for (unsigned m = beginM; m <endM ; ++m) {
                        ...
                        //cout<<"--Neighbor--"<<id<<"--flags[id]--"<<flags[id]<<endl;
                        //cout<<" "<<id;
                        //BOOST_VERIFY(id < graph.size());
                        if (flags[id]) { continue;}
                        flags[id] = true;
                        /*vector<NeighborX>::iterator it;
                        it = find(knn.begin(),knn.end(),neighbors[m]);
                        if(it!=knn.end()){
                            continue;
                        }*/
                        ++n_comps;
                        ...
                        //float dist = neighbors[m].dist;
                        //number++;
                        //cout<<"id="<<id<<"--dist="<<dist<<endl;
                        NeighborX nn(id, dist);
                        nn.M = actual_M(params.M,id);
                        ...
                        {
                            ++L;
                            //cout<<"temp update to "<<L<<endl;
                        }
                        if (r < L) {
                            knn[r].M = actual_M(params.M, id);
                            //cout<<"--"<<knn[r].M<<endl;
                            if (r < k) {
                                //cout<<k<<" update to "<<r<<endl;
                                k = r;
                            }
                        }
                    }
       ...
       virtual unsigned new_search (SearchOracle const &oracle, SearchParams const &params, unsigned *ids, float *dists, SearchInfo *pinfo) const {
            if (graph.size() > oracle.size()) {
                throw runtime_error("dataset larger than index");
            }
            if (params.P >= graph.size()) {
            ...
            vector<NeighborX> knn(params.K+params.P+100);
            vector<NeighborX> temp;
            vector<NeighborX> knn_result(params.K+params.P+100);
            vector<NeighborX> results;
            // flags access is totally random, so use small block to avoid
            // extra memory access
            boost::dynamic_bitset<> flags(graph.size(), false);
            ...
            for (unsigned trial = 0; trial < params.T; ++trial) {
                ...
                for (unsigned k = 0; k < L; ++k) {
                    ...
                    flags[e.id] = true;
                    e.flag = true;
                    e.dist = oracle(e.id); // compute hamming distance
                    e.m = 0;
                    e.M = actual_M(params.M, e.id);
                }
                sort(knn.begin(), knn.begin() + L);
                //knn.resize(L);
                cout<<"random select the initial id "<<endl;
                for(auto iter=knn.begin();iter != knn.end();++iter)
                {
                    cout<<"id="<<(*iter).id<<" dist="<<(*iter).dist<<endl;
                }    
                cout<<endl;
                cout<<" Knn size is ="<<endl;
                //knn_result.clear();
                cout<<"L is --"<<L<<endl;
                unsigned step = 0;
                step = L;
                ...
                while (k < L) {
                    cout<<"k----is---"<<k<<endl;
                    //cout<<"Knn size to "<<knn.size()<<endl;
                    auto &e = knn[k];
                    cout<<"search id="<<e.id<<endl;
                    if (!e.flag) { // all neighbors of this node checked
                        ++k;
                        continue;
                    }
                    ...
                    unsigned log = 0;
                    auto const &neighbors = graph[e.id];
                    cout<<"------graph[e.id].size()="<<graph[e.id].size()<<endl;
                    for (unsigned m = beginM; m < graph[e.id].size(); ++m) {
                        log = temp.size();
                        unsigned id = neighbors[m].id;
                        //cout<<"Neighbor.id="<<id<<endl;
                        //BOOST_VERIFY(id < graph.size());
                        //if (flags[id]) continue;
                        //flags[id] = true;
                        //
                        vector<NeighborX>::iterator it;
                        vector<NeighborX>::iterator its;
                        it = find(knn.begin(),knn.end(),neighbors[m]);
                        its = find(temp.begin(),temp.end(),neighbors[m]);
                        if(it!=knn.end() || its!=temp.end()){
                            continue;
                        }
                        ++n_comps;
                        float dist = oracle(id);
                        //float dist = neighbors[m].dist;
                        //cout<<"Neighbor.id="<<id<<" dist="<<dist<<endl;
                        NeighborX nn(id, dist);
                        nn.M = actual_M(params.M,id);
                        //unsigned r = UpdateKnnList(&knn[0],L,nn);
                        /*if(r<L){
                    ...
                    /*cout<<" before sort temp array is "<<endl;
                    ...
                    /*cout<<"after sort temp array is "<<endl;
                    for(auto iter=temp.begin();iter!=temp.end();++iter){
                        cout<<" "<<(*iter).id;
                    }
                    cout<<endl;
                    cout<<knn.size()<<"|"<<step<<"+ "<<temp.size()<<endl;*/
                    //knn_result.reserve(step+temp.size()+1);
                    knn_result.resize(L+temp.size()+1);
                    step = step+temp.size();

                    //cout<<knn_result.size()<<endl;
                    //

                    merge(knn.begin(),knn.begin()+L,temp.begin(),temp.end(),knn_result.begin());

                    /*cout<<"knn_result the id is "<<endl;
                    ...
                    knn.resize(knn_result.size()+1);

                    //knn.erase(knn.begin(),knn.end());
                    /*int i = 0;
                    for(auto iter=knn_result.begin();iter!=knn_result.end();++iter){
                        knn[i] = (*iter);
                        i++;
                    ...
                    //
                    copy(knn_result.begin(),knn_result.end(),knn.begin());
                    //knn = knn_result;
                    //knn.assign(knn_result.begin(),knn_result.end());
                    /*cout<<"knn array update is "<<endl;
                    for(auto iter=knn.begin();iter!=knn.end();++iter){
                        cout<<" "<<(*iter).id;
                    }
                    //cout<<endl;*/
